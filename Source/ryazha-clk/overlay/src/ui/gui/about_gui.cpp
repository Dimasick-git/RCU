...
void AboutGui::listUI()
{
    ...
    if(this->context->temps[RyazhaClkThermalSensor_AO] > 0) { // Only show if the value is valid (not -126, which means not patched)
...
void AboutGui::refresh()
{
    ...
    sprintf(strings[0], "%u/%u/%u", this->context->speedos[RyazhaClkSpeedo_CPU], this->context->speedos[RyazhaClkSpeedo_GPU], this->context->speedos[RyazhaClkSpeedo_SOC]);
    // This is how hekate does it
    sprintf(strings[1], "%u/%u/%u", this->context->iddq[RyazhaClkSpeedo_CPU], this->context->iddq[RyazhaClkSpeedo_GPU], this->context->iddq[RyazhaClkSpeedo_SOC]);
    ...
    s32 millis = context->temps[RyazhaClkThermalSensor_PLLX];
    sprintf(strings[3], "%u.%u °C", millis / 1000U, (millis % 1000U) / 100U);
    eristaPLLXItem->setValue(strings[3]);

    millis = context->temps[RyazhaClkThermalSensor_AO];
    if(millis > 0) {
    ...
    sprintf(strings[4], "%u.%u / %u mV", context->voltages[RyazhaClkVoltage_EMCVDD2] / 1000U, (context->voltages[RyazhaClkVoltage_EMCVDD2] % 1000U) / 100U, context->voltages[RyazhaClkVoltage_EMCVDDQ] / 1000);
    ramVoltItem->setValue(strings[4]);

    sprintf(strings[5], "%u.%u mV", context->voltages[RyazhaClkVoltage_Display] / 1000U, (context->voltages[RyazhaClkVoltage_Display] % 1000U) / 100U);
    dispVoltItem->setValue(strings[5]);

    sprintf(strings[6], "%u MB/s", context->partLoad[RyazhaClkPartLoad_RamBWAll]);
    ramBWItemAll->setValue(strings[6]);

    sprintf(strings[7], "%u MB/s", context->partLoad[RyazhaClkPartLoad_RamBWCpu]);
    ramBWItemCpu->setValue(strings[7]);

    sprintf(strings[8], "%u MB/s", context->partLoad[RyazhaClkPartLoad_RamBWGpu]);
    ramBWItemGpu->setValue(strings[8]);

    sprintf(strings[9], "%u MB/s", context->partLoad[RyazhaClkPartLoad_RamBWPeak]);
    ramBWItemMax->setValue(strings[9]);

    switch(context->temps[RyazhaClkThermalSensor_BQ24193]) {
